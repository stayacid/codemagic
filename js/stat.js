...
window.renderStatistics = function (ctx, names, times) {
    ...
    var sortTimes = times.sort(compareNumeric);

    //Берем максимальное и минимальное число, округляем до секунд
    var maxNumberSortTimes = sortTimes[sortTimes.length - 1];
    var maxRound = Math.round((Math.round(maxNumberSortTimes) / 1000) * 100) / 100;
    var minRound = Math.round((Math.round(sortTimes[0]) / 1000) * 100) / 100;
    ...
    for(var i = 0; i < times.length; i++) {
        ...
        var randomOpacity = Math.floor(Math.random() * 10);
